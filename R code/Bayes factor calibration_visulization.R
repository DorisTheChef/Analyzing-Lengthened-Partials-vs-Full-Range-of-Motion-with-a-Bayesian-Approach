...
# check calibration
run_bf_calibration_details <- function(n_participants = 25, n_sim = 10) {
  
  all_res <- map_dfr(1:n_sim, ~run_one_bf_sim(n_participants))
  
  # Column 3: average posterior model probability
  # This averages the posterior probability assigned to the true model
  avg_posterior_model_probability <- mean(
    ifelse(
      all_res$true_model == "H1",
      all_res$post_H1,
      all_res$post_H0
    )
  )
  
  # Column 4: posterior allocated to H1 when H1 is true
  avg_posterior_H1_when_H1_true <- mean(
    all_res$post_H1[all_res$true_model == "H1"]
  )
  
  # Column 5: posterior allocated to H0 when H0 is true
  avg_posterior_H0_when_H0_true <- mean(
    all_res$post_H0[all_res$true_model == "H0"]
  )
  
  tibble(
    N = n_participants,
    avg_posterior_model_probability = avg_posterior_model_probability,
    avg_posterior_H1_when_H1_true = avg_posterior_H1_when_H1_true,
    avg_posterior_H0_when_H0_true = avg_posterior_H0_when_H0_true
  )
}
set.seed(261)

bf20 <- run_bf_calibration_details(20, n_sim = 10)
bf25 <- run_bf_calibration_details(25, n_sim = 10)
bf30 <- run_bf_calibration_details(30, n_sim = 10)
